...
docker run -d --name redis1 -p 6379:6379 --restart always --privileged -v "$(Get-Location).Path\redis\db1\conf:/usr/local/etc/redis" -v "$(Get-Location).Path\redis\db1\data:/data" redis redis-server /usr/local/etc/redis/redis.conf

docker run -d --name redis2 -p 6380:6379 --restart always --privileged -v "$(Get-Location).Path\redis\db2\conf:/usr/local/etc/redis" -v "$(Get-Location).Path\redis\db2\data:/data" redis redis-server /usr/local/etc/redis/redis.conf
